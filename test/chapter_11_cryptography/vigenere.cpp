...
TEST(vigenere, DISABLED_encrypt_empty_string) {
    const auto text{ "" };
    EXPECT_EQ(vigenere{}.encrypt(text), text);
}
TEST(vigenere, DISABLED_encrypt_string_with_no_alphas) {
    const auto text{ "1234567890!?@#" };
    EXPECT_EQ(vigenere{}.encrypt(text), text);
}

TEST(vigenere, DISABLED_encrypt_and_decrypt_empty_string) {
    const auto text{ "" };
    vigenere cipher{};
    auto ret{ cipher.encrypt(text) };
    EXPECT_EQ(cipher.decrypt(ret), text);
}
TEST(vigenere, DISABLED_encrypt_and_decrypt_string_with_no_alphas) {
    const auto text{ "1234567890!?@#" };
    vigenere cipher{};
    auto ret{ cipher.encrypt(text) };
    EXPECT_EQ(cipher.decrypt(ret), text);
}
TEST(vigenere, DISABLED_encrypt_and_decrypt) {
