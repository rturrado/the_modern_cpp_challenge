...
TEST(caesar, encrypt_string_with_no_alphas) {
    const auto text{ "1234567890!?@#" };
    EXPECT_EQ(caesar{}.encrypt(text), text);
}

TEST(caesar, encrypt_and_decrypt_empty_string) {
    const auto text{ "" };
    caesar cipher{};
    auto ret{ cipher.encrypt(text) };
    EXPECT_EQ(cipher.decrypt(ret), text);
}
TEST(caesar, encrypt_and_decrypt_string_with_no_alphas) {
    const auto text{ "1234567890!?@#" };
    caesar cipher{};
    auto ret{ cipher.encrypt(text) };
    EXPECT_EQ(cipher.decrypt(ret), text);
}
TEST(caesar, encrypt_and_decrypt) {
