...
TEST(delete_directory_entries_older_than, DISABLED_in_dir_path_does_not_exist) {
...
TEST(delete_directory_entries_older_than, DISABLED_in_dir_path_is_not_a_directory) {
    std::ostringstream oss{};
...
TEST(delete_directory_entries_older_than, DISABLED_now) {
    std::ostringstream oss{};
    const auto resource_folder_path{ tmcppc::env::get_instance().get_resource_folder_path() };
...
TEST(delete_directory_entries_older_than, DISABLED_five_hundred_years_ago) {
    std::ostringstream oss{};
    const auto resource_folder_path{ tmcppc::env::get_instance().get_resource_folder_path() };
    const auto in_dir_path{ resource_folder_path / "sample_folder" };
