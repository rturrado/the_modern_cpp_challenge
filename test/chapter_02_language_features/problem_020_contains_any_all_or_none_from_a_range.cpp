...
TEST(contains_any, empty_container) { EXPECT_FALSE(contains_any(std::vector<int>{}, 1, 3, 5)); }
TEST(contains_all, empty_container) { EXPECT_FALSE(contains_all(std::vector<int>{}, 1, 3, 5)); }
TEST(contains_none, empty_container) { EXPECT_TRUE(contains_none(std::vector<int>{}, 1, 3, 5)); }

TEST(contains_any, non_empty_container) {
    EXPECT_TRUE(contains_any(std::vector<int>{ 1, 1, 2, 3, 5, 8 }, 1, 3, 5));
}
TEST(contains_all, non_empty_container) {
    EXPECT_TRUE(contains_all(std::vector<int>{ 1, 1, 2, 3, 5, 8 }, 1, 3, 5));
}
TEST(contains_none, non_empty_container) {
    EXPECT_FALSE(contains_none(std::vector<int>{ 1, 1, 2, 3, 5, 8 }, 1, 3, 5));
}

TEST(contains_any, view_of_even_numbers) {
